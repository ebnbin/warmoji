...
import { Ability, Cd, Frozen, Owner, Rushing, WindupState } from '../components'
import { cooldownMul } from '../utils/amp'
import type { Sim } from '../sim'

/** 冷却只在能力空闲时走：蓄力中、冲刺中都按在满值上，动作做完才开始计时 */
export function tickCooldowns(sim: Sim): void {
  const dt = sim.wdtMs
  for (const e of query(sim.world, [Ability, Cd, Frozen, Owner])) {
    if (Frozen.v[e]) continue
    const o = Owner.eid[e]!
    const held = WindupState.until[e]! > 0 || (Rushing.active[o] === 1 && Rushing.skill[o] === e)
