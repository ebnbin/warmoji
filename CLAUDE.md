...
- 玩法数值随实体归包：characters/enemies/weapons/items/maps/run/battle 各自持有；全局锚定 VIEW/UNIT/TAP_SLOP 在 `src/lib/units.ts`——调数值先定位实体包
- 会持续增行的实体数据行放各包 `*.json`，按设计单位书写（`"Nu"` 格 / `"Ndeg"` 角度 / `"0x…"` 颜色），`lib/hydrate.ts` 是唯一换算边界，跨实体用 ID 引用（加载时解析、配错即抛）；行为定义（abilities）、锚定、少量调参旋钮留 TS
