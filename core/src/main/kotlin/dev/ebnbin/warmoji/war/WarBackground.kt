...
class WarBackground(
    ...
    fun draw(batch: Batch, parentAlpha: Float, camera: Camera) {
        batch.end()
        ...
        val area = Rectangle(0f, 0f, columns.toFloat(), rows.toFloat())
        val scissor = Rectangle()
        ScissorStack.calculateScissors(camera, batch.transformMatrix, area, scissor)
        batch.flush()
        if (!ScissorStack.pushScissors(scissor)) return
        tileList.forEach { tile ->
            batch.draw(tile.texture, tile.x, tile.y, 0.5f, 0.5f, 1f, 1f, tile.scale, tile.scale, tile.rotation, 0, 0,
