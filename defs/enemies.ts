...
const KNIGHT = {
  ...
  emoji: '1f3c7',
  name: '骑士',
  desc: '先打马再打人：坐骑替它扛下前面的伤害，骑马时会蓄力冲锋撞人；马倒了就下马慢步挥剑',
  size: 1.45,
  radius: 0.52,
  hp: 70,
  speed: 2.2,
...
const CRAB = {
  ...
  emoji: '1f980',
  name: '盾蟹',
  desc: '举着大钳子横着走，正面来的攻击全被挡下，得绕到侧面、背后，或者把它掀翻再打',
  size: 1.35,
  radius: 0.5,
  hp: 70,
  speed: 1.1,
...
const RACCOON = {
  ...
  emoji: '1f99d',
  name: '浣熊',
  desc: '一碰到队员就偷走他的主动技能自己用，被偷的技能冷却重新走；打死它，技能立刻还回来',
  size: 1.2,
  radius: 0.45,
  hp: 55,
  speed: 2.4,
...
const SIREN = {
  ...
  emoji: '1f9dc',
  name: '海妖',
  desc: '远远地唱歌，歌声打中的队员被魅惑，不由自主地朝它走去',
  size: 1.3,
  radius: 0.48,
  hp: 50,
  speed: 1.6,
  ...
  abilities: [
    {
      ...
      aim: 'nearest',
      range: 8,
      shape: { kind: 'bolt', projectile: { emoji: '1f3b6', size: 0.45, radius: 0.16, speed: 5, rotationOffsetDeg: 0 }, lifeMs: 3000 },
...
const TREE = {
  ...
  emoji: '1f332',
  name: '古树',
  desc: '树妖的种子长成的古树，朝附近的人甩出缠根；树妖能瞬移到它身边',
  size: 1.9,
  radius: 0.7,
  hp: 160,
  speed: 0,
...
const PYLON = {
  ...
  emoji: '1f50b',
  name: '护盾塔',
  desc: '母机核心立起的供能塔：只要还有一座立着，核心就刀枪不入',
  size: 1.3,
  radius: 0.5,
  hp: 150,
  speed: 0,
...
const FOREST_BOSS = {
  ...
  emoji: '1f333',
  name: '树妖',
  desc: '黑森林头目：一路撒下六秒后长成古树的种子，瞬移到离你最近的古树旁把周围的人缠住，孢子扇幕封走位；击退免疫',
  size: 3.4,
  radius: 1.1,
  hp: 6500,
  speed: 1.0,
...
const RIVER_BOSS = {
  ...
  emoji: '1f40a',
  name: '巨鳄',
  desc: '奔流头目：一口吞下贴近的队员慢慢消化，打它够疼才吐出来；死亡翻滚时霸体横扫；水弹扇幕与水柱，击退免疫',
  size: 3.4,
  radius: 1.1,
  hp: 6200,
  speed: 1.3,
...
const FACTORY_BOSS = {
  ...
  emoji: '1f916',
  name: '母机核心',
  desc: '工厂头目：立起护盾塔，塔还立着核心就打不动；追踪导弹紧咬不放；血过半变成狂暴形态再立两座塔，环扫激光与重锤齐下；击退免疫',
  size: 3.4,
  radius: 1.1,
  hp: 6500,
  speed: 1.2,
...
const DAYNIGHT_BOSS = {
  ...
  emoji: '1f317',
  name: '晦明',
  desc: '晨昏原野头目：把最近的队员拖进暗影界单挑五秒，界外的人插不了手；隔一阵入夜化作月蚀形态，暗夜里一圈圈恐惧；日冕环爆与月华坠，击退免疫',
  size: 3.4,
  radius: 1.1,
  hp: 6300,
  speed: 1.25,
...
const ICE_BOSS = {
  ...
  emoji: '1f9a2',
  name: '冰羽天鹅',
  desc: '浮冰头目：扇出减速的冰羽；把一片队员冻进冰里两秒（冻住的打不了也挨不了打）；冰块砸落铺开打滑的冰面；生命归零时缩进冰蛋，六秒内打不碎就满血复生',
  size: 3.3,
  radius: 1.05,
  hp: 5600,
  speed: 1.35,
  ...
  abilities: [
    {
      ...
      aim: 'nearest',
      damage: 8,
      shape: { kind: 'bolt', projectile: { emoji: '1fab6', size: 0.5, radius: 0.17, speed: 3.5, rotationOffsetDeg: 45 }, lifeMs: 4500 },
    ...
    },
    {
      ...
      aim: 'nearest',
      damage: 20,
      shape: { kind: 'drop', targets: 3, emoji: '1f9ca', size: 1, fromAbove: 4, dropMs: 240, staggerMs: 90 },
