...
// 试炼场（地图页进入，run.sandbox = true）：免死无时限的沙盒，
// 敌人 / 角色 + 规模 / 难度 / 攻速 / 无敌 都能在场内自由切换。
// 队长固定为沙盒专用队长（编制 8、无限金币、永远满豆、无增益），不可更改。
// 以下为当前选择状态（模块级，跨场景重启保留）。
//
// 战斗侧（两套实现）只读这里的 7 个符号：spawnParams / INVINCIBLE_HP /
// sandboxEnemySet / sandboxDifficulty / sandboxFireRate / sandboxInvincible / sandboxLevel。
// 它们的签名是**对战斗侧的契约**：试炼场的形态怎么变，都不该让任何一套战斗跟着改一行。

const enemies = new Set<string>()
// 最少 1 人、最多 = 沙盒队长编制（8）；开局值由文件末尾的默认预设写入
...
export function applySandboxPreset(id: string): void {
  ...
  setSandboxInvincible(true) // 免死：否则重载下队伍几秒就没，测不到稳态
...
export function toggleSandboxCharacter(id: CharacterId): void {
  if (roster.includes(id)) {
    ...
    if (roster.length >= CAPTAINS[SANDBOX_CAPTAIN].teamSize) return // 最多 8 人
    roster = [...roster, id]
  }
  presetId = undefined
}
...
// ── 开局 ──────────────────────────────────────────────────
/** 进入试炼场的开局阵容：勾选角色截到沙盒队长编制上限（8），至少留 1 人兜底 */
