...
export class PromoteScene extends Phaser.Scene {
  ...
  create(): void {
    ...
    const dragged = (): boolean => this.grid?.wasDragged ?? false

    addTeamFrame(
      this,
      L,
      this.origin,
      this.mode === 'formation' ? '布置阵型' : isInitialWave(this.run) ? '组建队伍' : '队伍整编',
      this.stepBanner(),
      res,
    )
  ...
  private confirmLabel(): string {
    ...
    return nextAfterTeam(this.run) === 'shop' ? '前往商店' : '开战'
  }
  ...
  private renderCenterDetail(res: number): void {
    ...
    const end = renderStatGroups(this, this.detailView, this.detailRect.w, center, items, res, start)
