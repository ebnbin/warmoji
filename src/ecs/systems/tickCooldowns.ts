...
import { Ability, Cd, Frozen, Owner, Rushing, Thrown, WindupState } from '../components'
...
export function tickCooldowns(sim: Sim): void {
  ...
  for (const e of query(sim.world, [Ability, Cd, Frozen, Owner])) {
    ...
    const o = Owner.eid[e]!
    const held = WindupState.until[e]! > 0 || Thrown.n[e]! > 0 || (Rushing.active[o] === 1 && Rushing.skill[o] === e)
