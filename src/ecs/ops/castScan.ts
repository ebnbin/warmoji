...
import { Ability, CastRequest, Cooldown, Disarmed, Fired, Frozen, Manual } from '../components'
...
export function castScan(sim: Sim, comp: object, cast: (eid: number) => boolean | void): void {
  ...
  for (const e of [...query(sim.world, [Ability, comp])]) {
    ...
    if (Cooldown.baseMs[e]! > 0) Cooldown.left[e] = Cooldown.baseMs[e]! * cooldownMul(sim, e)
  }
}
