...
- src 按业务域分包：boot/lib/screen/emoji/audio/characters/captains/enemies/abilities/items/pickups/projectiles/maps/run/battle/menu/debug；实体包收拢该实体的 def 类型、注册表、纯逻辑与运行时结构/机器；纯逻辑与表现文件同包不同文件，import phaser 仅限 eslint 白名单（eslint.config.js；DOM/WebAudio 越界靠约定），单测只测纯文件、与被测同居
